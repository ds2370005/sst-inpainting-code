"""Train the anomaly-inpainting network on Himawari or synthetic data."""
...
class SyntheticAnomalyDataset(Dataset[dict[str, torch.Tensor]]):  # 学習処理: 形状確認用の異常補完サンプルを乱数で作る
    """Random tensors with the same shapes as anomaly-inpainting batches."""
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        target_anomaly = 0.25 * torch.randn(1, height, width)
        target_sst = (weekly_average + target_anomaly).clamp(-1.0, 1.0)

        temporal_noise = 0.1 * torch.randn(1, time_steps, height, width)
        sst_volume = target_sst.unsqueeze(1).repeat(1, time_steps, 1, 1)
...
def create_synthetic_anomaly_loader(
    config: Mapping[str, Any],
    *,
    num_samples: int,
) -> DataLoader[dict[str, torch.Tensor]]:  # 学習処理: 異常補完用の疑似バッチをDataLoader化する
...
def build_anomaly_training_components(
    config: Mapping[str, Any],
    device: torch.device,
    ...
    *,
    ...
    for batch_index, raw_batch in enumerate(loader):
        ...
        if use_adversarial:
            ...
            with autocast_context():
                real_logits = discriminator(batch["assimilation_sst"])
                fake_logits = discriminator(pred_sst.detach())
                loss_d = discriminator_loss(real_logits, fake_logits)
            loss_d.backward()
            optimizer_d.step()
        else:
            loss_d = pred_sst.new_zeros(())

        if use_adversarial:
            set_requires_grad(discriminator, False)
        optimizer_g.zero_grad(set_to_none=True)
        with autocast_context():
            loss_rec = masked_mse_loss(pred_sst, target_sst, target_mask)
            if use_adversarial:
                fake_logits_for_g = discriminator(pred_sst)
                loss_adv = generator_adversarial_loss(fake_logits_for_g)
            else:
                loss_adv = pred_sst.new_zeros(())
...
def main() -> None:  # 学習処理: 実データまたはsyntheticで異常補完を学習する入口
    args = parse_args()
    ...
    if args.synthetic:
        ...
        loader = create_synthetic_anomaly_loader(
            config,
            num_samples=synthetic_num_samples,
        )
    else:
        assert args.data_root is not None
        loader = create_himawari_loader(args.data_root, config, stage="anomaly")
        print(
    ...
    checkpoint_dir = Path(config["paths"]["checkpoint_dir"]) / "anomaly"
